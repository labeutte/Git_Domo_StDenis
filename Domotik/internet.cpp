...
internet::internet() {
   ...
   if (rootNode == nullptr) {
      ...
      if (element != nullptr) {
         ...
         attribText = element->Attribute("portFtp");
         portFtp = atoi(attribText);
         //Site2
         attribText = element->Attribute("url2");
         strcpy(urlSite2, attribText);
         attribText = element->Attribute("logMdp2");
         strcpy(logPwd2, attribText);
         attribText = element->Attribute("portFtp2");
         portFtp2 = atoi(attribText);
      }
   }
   if (Debug) {
      cout << "FTP1 : " << urlSite << " - " << logPwd << endl;
      cout << "FTP2 : " << urlSite2 << " - " << logPwd2 << endl;
   }
}
...
void internet::putFTP(string img) {
   ...
   if (r) { // Il y a du réseau
      /*****FTP en ligne de Cde pour ORANGE*****/
      string stftp;
      char command[250];
      if (Debug) {
      ...
      strcpy(site, urlSite2); //intermediaire pour ne pas modifier urlSite
      strcpy(urlFTP2, strcat(site, nomImg.c_str()));
      if (Debug) {
      ...
      if (stat(origin, &file_info)) {
         if (Debug)
            fprintf(stderr, "FTP2 - Couldn't open '%s': %s\n", origin, strerror(errno));
      }
      ...
      if (curl) {
         ...
         // specify target
         curl_easy_setopt(curl, CURLOPT_URL, urlFTP2);
         curl_easy_setopt(curl, CURLOPT_USERPWD, logPwd2);
         curl_easy_setopt(curl, CURLOPT_PORT, portFtp2);
         ...
         couleur("33");
         if (res != CURLE_OK)
            fprintf(stderr, "\ninternet.putFTP : curl_easy_perform() failed: %s\n", curl_easy_strerror(res));
         else
            cout << endl << "Image envoyee sur Freebox" << endl;
         couleur("37");
         ...
         cout << "Pb ouverture curl sur FTP2" << endl;
